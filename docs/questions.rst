...
**用户需要自己学习和了解 Pyarmor 的各个选项以及使用方法，并根据自己的需求选择正确的选项，而不是让 Pyarmor 开发组去了解自己的项目结构和加密需求，然后告诉自己需要使用什么选项**
