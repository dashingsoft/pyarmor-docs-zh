...
.. important::
   ...
   如果使用 :option:`--enable-rft` ，请同时参考 :doc:`../topic/rftmode` 。默认的重构方式使用简单，但是可能有大量的名称并没有进行重命名。如果不能满足需要，请启用配置选项 **rft-auto-include**

   如果使用 :option:`--enable-bcc` ，请同时参考 :doc:`../topic/rftmode` 确认重要的 Python 代码被转换成为 C 代码
...
需要注意的是 `BCC 模式` 不会把模块级别的代码转换为 C 代码，所以如果使用了 :option:`--enable-bcc` ，一般不要使用模块级别的代码。
