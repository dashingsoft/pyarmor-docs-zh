...
在需要离线使用 Pyarmor 的第一台设备上面安装 Pyarmor 8.2 以上版本，然后首先运行下面的命令设定该设备的组信息::
...
生成离线设备的注册文件需要的初始登记之后生成的 :term:`注册文件` 以及设备的组信息文件 ``pyarmor-group-file.1`` ，还需要互联网连接。把设备的组信息文件拷贝到初始登记的机器上，存放在指定目录 ``.pyarmor/group/`` 下面，然后运行下面的命令::
