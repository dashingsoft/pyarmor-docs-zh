...
发布加密脚本给客户只需要把输出路径 `dist` 的所有文件拷贝过去即可，客户并不需要安
装 PyArmor
...
但是直接在脚本中添加认证代码可能会影响调试，并且这些代码有时候必须在加密环境下才
能运行。所以推荐的方式是 :ref:`使用插件扩展认证方式` 。插件是一个普通的脚本，等
价于把认证代码单独提取出来，然后在加密的时候注入到被加密的脚本中去，这样不需要对
原来的脚本进行任何修改。
