...
只适用于 Python 3.7 以及其后的版本。

模式 6 是模式 5 的增强版，主要是保护模块字典不会显示任何内容。例如，如果模块
...
如果模块使用模式 6 加密，创建和删除模块全局变量只能在模块级别的代码中执行，换言
之，一旦模块初始化完成之后，就不能增加和删除模块级别的变量。并且这个模块被释放之
后，模块字典里面的内容可能不会被释放，从而造成内存泄露。
