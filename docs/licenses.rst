...
版本的是不要要保留注册信息，也就是说，不要删除存放注册文件信息的路径，默认情况下
注册信息是存放在 :file:`~/.pyarmor`

如果老的许可证已经设定过产品名称，那么直接运行下面的命令升级::

    $ pyarmor reg -u pyarmor-regcode-xxxx.txt

如果老的许可证没有设定产品名称，按照新的 `Pyarmor 最终用户许可协议`_ ，需要为每
一个许可证指定产品名称。这也意味着，如果老的许可证是被用于多个产品的话，升级之后
就只能用于其中的一个，其他产品还需要购买新的许可证。
