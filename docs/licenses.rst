...
试用版本永不过期，但是有功能方面的限制，这些限制可以通过不同的许可证进行解锁。Pyarmor 目前共有四种许可模式:
...
5. 允许合理使用
   ...
   :term:`Pyarmor 专家版` 许可证可以少量的使用于 CI/CD 管线中，每月不超过 100 次（主要用于调试使用）

   :term:`Pyarmor 管线版` 许可证在 CI/CD 管线中注册有请求速率的限制，并且每月总注册次数也有限制
...
2. 判断多个不同产品是否同一个产品
   ...
   a. 第一种情况，满足下列条件，则可以认为 B 和 A 可以使用同一个许可证
      ...
      例如 Pyarmor Basic 版本 和 Pyarmor Pro 版本 可以使用同一个许可证，因为

      - Pyarmor Pro 包含 Pyarmor Basic 的所有功能
      - Pyarmor Pro 的额外功能是不可逆加密模式，这是对 Pyarmor Basic 加密功能的增强

   b. 第二种情况，下列情况需要为 A 和 B 单独购买许可证
   ...
   c. 第三种情况，满足下列条件，则可以认为 B 和 A 可以使用同一个许可证

      - B 是针对 A 产品的功能补充

      例如 A 是 CAD 文件编辑器，B 是 CAD 文件转换器，用于将 CAD 文件转换成为 PDF 格式
...
.. _how_to_upgrade_license:

如何升级Pyarmor许可证
---------------------

目前并不支持补差价升级许可证，例如升级 Pyarmor 基础版到 Pyarmor 专家版许可证等

例外的情况主要有

- 老版本许可证（应用于 Pyarmor <= 7.x）在满足一定条件下可以升级到 Pyarmor 基础版许可证
- 如果从 Pyarmor 8.0 升级到 Pyarmor 9.0，许可证也需要进行相应的重新注册

具体操作步骤请参考 :doc:`升级 Pyarmor 之后的许可证兼容性 <how-to/register#pyarmor>`

简单来说，如果能够根据文档说明升级成功，那么就是可以升级。如果升级过程中提示出现错误，那么就是无法升级。Pyarmor 开发组不处理升级许可证的请求，有可能不会回复这种类型的邮件。

问与答
------

**我的产品刚刚开始销售，是否可以使用 Pyarmor 试用版进行加密保护我的产品**
