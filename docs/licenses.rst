...
支付成功之后，在一个工作日之内激活文件会发送到注册邮箱，请按照激活文件中的方法
和步骤完成注册和激活。
...
如果需要使用信用卡或者 Paypal 进行支持，在浏览器中 MyCommerce 官网的购物车进行购
买，这里的价格单位是美元，并且不能开国内增值税发票，只提供国际通用的电子发票
...
然后在一台已经注册过 Pyarmor 的机器上安装最新版本的 Pyarmor 8.0+，注意卸载原来的
版本的是要保留原来的注册信息，也就是说，不要删除存放注册文件信息的路径，默认情况
下注册信息是存放在 :file:`~/.pyarmor`

如果老的许可证已经设定过产品名称，那么直接运行下面的命令升级::

    $ pyarmor reg -u pyarmor-regcode-xxxx.txt

如果老的许可证没有设定产品名称，但是只用于内部使用或者个人使用，也可以直接升级::

    $ pyarmor reg -u pyarmor-regcode-xxxx.txt

而对于商业应用，按照新的 `Pyarmor 最终用户许可协议`_ ，需要为每一个许可证指定产
品名称。这也意味着，如果老的许可证是被用于多个产品的话，升级之后就只能用于其中的
一个，其他产品还需要购买新的许可证。
...
支付成功之后，在一个工作日之内会激活文件会发送到注册邮箱，请按照激活文件中的方法
和步骤完成升级操作。
