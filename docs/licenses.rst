...
只有当运行 Docker 容器的设备 (Host) 可以使用集团版许可证的时候，集团版许可证可以用在 Docker 容器中，否则 Docker 容器无法使用集团版许可证，具体使用方法请参考 :doc:`how-to/register` 中的 `运行不受限制的 Docker 容器`
...
.. [#] 这个功能是从 Pyarmor 8.3 开始支持，集团版许可证可以运行不受限制的 Docker，基础版和专家版一天之内最多运行 Docker 容器 100 次。
...
..
  但是对于老用户购买专家版许可证有一定折扣，折扣后的价格 = 原价格 * (1 - 折扣率)

  .. list-table:: 表-3. 老用户购买专家版许可证折扣率
     :header-rows: 1

     * - 购买老的许可证日期
       - 折扣率（%）
       - 备注
     * - 2022-06-01 和 2023-05-31 之间
       - 40
       -
     * - 2021-06-01 和 2022-05-31 之间
       - 30
       -
     * - 2021-05-31 之前
       - 20
  ...
  请把订单编号发送到 pyarmor@163.com 请求相应的优惠优惠券，注意必须使用订单对应的注册邮箱发送请求，否则无效。
