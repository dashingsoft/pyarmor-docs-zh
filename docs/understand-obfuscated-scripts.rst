...
使用 `公共密钥箱` 是无法真正的限制加密脚本的使用期限或者绑定到某一台指定设备上，
因为别人同样可以使用 `公共密钥箱` 为你的加密脚本生成合法的许可，使用相同的密钥箱
为加密脚本生成的许可是通用的。
...
.. important::
   ...
   密钥箱文件可能会对恢复加密脚本有帮助，请不要把专用的 `私有密钥箱` 拷贝给其他
   任何人。
...
* `运行辅助包`_ 必须在任何 Python 路径下面，确保 `引导代码`_ 能被正确导入。运行
  辅助包 `pytransform` 是一个正常的 Python 包，它的导入方式完全使用 Python 的包
  导入机制。如果存在多个同名的包或者模块 `pytransform` 的时候，请确保导入的是和
  加密脚本匹配的运行辅助包。超级模式和其他模式的运行辅助包完全不一样，但是包名称
  一样，如果导入不匹配的运行辅助包，那么加密脚本将无法正常运行。
