...
``--noconfirm`` ， ``-n`` ， ``--name`` ， ``--distpath`` ， ``--distpath`` ，因
为这些会被 `pack`_ 命令内部使用。
...
``--output`` 以及 ``-x`` 中指定的额外选项来调用 `pyarmor obfuscate` 。当打包一个
工程的时候，也就是说最后一个参数是工程路径而不是脚本的时候， `pack`_ 会直接调用
命令 `build`_ 加密脚本，选项 ``-x`` 的值会被忽略，这时候加密选项的控制是通过配置
工程来实现。
